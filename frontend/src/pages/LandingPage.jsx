...
const Brand = ({ footer = false }) => <span className="landing-brand"><Logo size={footer ? 28 : 32} textSize={footer ? 16 : 18} textColor={footer ? '#FFFFFF' : '#111827'} /></span>;
const features = [
...
export default function LandingPage() {
  ...
  return <div className="landing-page">
    ...
    <main>
      ...
      <section className="how-section" id="how"><h2>Get started in minutes</h2><div className="steps">{[['Create your account','Sign up as staff in seconds. Admins can set up the full workspace.'],['Add your inventory','Add products, categories, and suppliers — import or enter manually.'],['Track and report','Monitor stock levels, get low-stock alerts, and generate reports.']].map(([title,body],index) => <article className="step" key={title}><div className="step-top"><span>{index+1}</span>{index<2&&<i/>}</div><h3>{title}</h3><p>{body}</p></article>)}</div></section>
      <section className="stats-section" id="pricing"><div>{[['10,000+','Products tracked daily'],['99.9%','Uptime reliability'],['3 Roles','Admin, Manager, Staff'],['Free','No subscription needed']].map(([number,label],index) => <article key={label}><strong>{number}</strong><span>{label}</span>{index<3&&<i/>}</article>)}</div></section>
    ...
    <footer className="landing-footer"><div><Brand footer /><p>© 2026 InvenTrack. All rights reserved.</p></div><nav><a href="#privacy">Privacy Policy</a><a href="#terms">Terms of Service</a><a href="mailto:support@inventtrack.local">Contact</a></nav></footer>
  </div>;
}

function DashboardPreview() {
  return <div className="dashboard-preview"><header><div><LogoMark size={24}/><strong>Dashboard</strong></div><i/></header><div className="preview-content"><div className="preview-kpis">{[['Total Products','11'],['Stock Value','₹19,19,897'],['Low Stock','5'],['Categories','4']].map(([label,value],index) => <div key={label}><span>{label}</span><strong>{value}</strong><i className={`preview-dot dot-${index}`}/></div>)}</div><div className="preview-panels"><section><header>Products by Category</header><div className="preview-chart">{[44,72,55,86,62,38].map((height,index) => <i key={index} style={{height:`${height}%`}}/>)}</div></section><section><header>Low Stock Alerts</header>{[['Wireless Mouse','3 remaining'],['Printer Paper','8 remaining'],['Office Chair','4 remaining']].map(([name,count]) => <div className="preview-alert" key={name}><span><b>{name}</b><small>{count}</small></span><em>LOW STOCK</em></div>)}</section></div></div></div>;
}
