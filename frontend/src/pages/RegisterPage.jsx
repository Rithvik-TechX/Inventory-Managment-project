...
const initial = { firstName: '', lastName: '', email: '', phone: '', company: '', department: 'Warehouse', role: 'STAFF', password: '', confirm: '' };
...
export default function RegisterPage() {
  ...
  const submit = async () => {
    ...
    try {
      await api.post('/auth/signup', { username: form.email, email: form.email, password: form.password, role: form.role, name: `${form.firstName} ${form.lastName}` });
  ...
  return (
    <main className="auth-page auth-page--center">
      <section className="register-card">
        <Link to="/login" className="auth-logo"><span className="auth-logo__mark">N</span><span>NexaStock</span></Link>
        ...
        <div className="register-form">
          ...
          {step === 1 && <div className="form-grid"><Field wide label="Company name" value={form.company} onChange={set('company')} /><Select label="Department" value={form.department} onChange={set('department')} options={['Warehouse','Sales','Admin','Finance']} /><Select label="Requested role" value={form.role} onChange={set('role')} options={['STAFF','MANAGER']} /></div>}
          ...
          {step === 3 && <div className="review-card"><div className="review-avatar">{form.firstName[0]}{form.lastName[0]}</div><h2>{form.firstName} {form.lastName}</h2><p>{form.email}</p><dl><div><dt>Company</dt><dd>{form.company}</dd></div><div><dt>Department</dt><dd>{form.department}</dd></div><div><dt>Role</dt><dd>{form.role}</dd></div></dl></div>}
...
function Select({ label, options, ...props }) { return <label><span>{label}</span><select {...props}>{options.map(option => <option key={option}>{option}</option>)}</select></label>; }
